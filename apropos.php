<?php

?>

<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <title>A propos de Fabrice Labit | Photographe de mariage</title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link href="https://fonts.googleapis.com/css?family=Comfortaa:300,400,700|Roboto:300" rel="stylesheet">
    <link rel="stylesheet" href="http://code.jquery.com/ui/1.12.1/themes/base/jquery-ui.css">
    <link rel="stylesheet" href="css/normalize.css">
    <link rel="stylesheet" href="css/main.css">
    <link rel="icon" type="image/gif" href="images/logo_icon.gif"/>
    <meta name="description"
          content="Photographe professionnel spécialisé dans les portraits, je couvre les mariages en mode photoreportage."/>
    <!-- balises Open Graph Facebook -->
    <meta property="og:url" content="http://www.fabrice-labit-mariage.com/index.php"/>
    <meta property="og:title" content="Fabrice Labit | Photographe de mariage"/>
    <meta property="og:description"
          content="Votre photographe Fabrice Labit fera de votre mariage ou engagement le meilleur souvenir qu'il soit grâce à sa discrétion et la qualité de ses photos."/>
    <meta property="og:image" content="http://www.fabrice-labit-mariage.com/images/...."/>
</head>
<body>
<?php
require_once 'views/header.php';
?>

<main class="wrapper" id="apropos">
    <h2>A propos de moi</h2>
    <div class="row">
        <img src="images/photo-fabrice-labit-big.jpg" alt="photo Fabrice Labit" class="col-4">
        <div class="col-8">
            <p>Il est très important pour ce jour unique d'avoir l'assurance de faire appel à un photographe professionnel
                pour que votre journée soit capturée dans les meilleures conditions quelles que soient les circonstances. En
                observateur discret, je couvre les mariages en mode photoreportage. </p>
            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed id eros elit. Phasellus luctus eros leo, eu
                elementum ipsum suscipit id. Integer dictum, quam lacinia lacinia tincidunt, est erat suscipit massa, vitae
                sagittis nisi ligula ut augue. Cras libero est, vestibulum eu sollicitudin a, interdum a enim. Integer
                facilisis laoreet dolor, accumsan porttitor orci mattis in. Sed et massa ac erat facilisis rutrum. Curabitur
                dapibus ligula vel mollis rutrum. Integer congue dolor orci, sit amet euismod metus placerat.</p>
            <p>Aliquam diam turpis, fringilla nec metus a, ultricies convallis diam. Pellentesque id nibh augue. Fusce
                euismod purus laoreet vulputate sagittis. Sed varius volutpat libero, consequat iaculis velit facilisis non.
                Nunc vitae sem ex. Curabitur elit purus, lobortis nec suscipit sit amet, venenatis a elit. Aenean tempor
                risus nec nisl porta, sed imperdiet risus fringilla. Duis aliquet pulvinar mauris sit amet vestibulum. Proin
                mollis sollicitudin faucibus. Maecenas sit amet placerat ex. Phasellus quis nulla sed risus pretium
                fringilla in eu ligula.</p>
            <p>Donec vel ultrices urna, at eleifend sapien. In condimentum nisl eget justo iaculis viverra. Praesent maximus
                ex elit, sed lacinia velit hendrerit in. Mauris sed scelerisque sapien. Proin eu massa ullamcorper,
                imperdiet lorem sed, cursus dui. Curabitur facilisis aliquam eleifend. Integer bibendum, felis ac finibus
                pellentesque, dui velit semper velit, quis vestibulum nulla leo sed magna. Sed rhoncus neque in convallis
                sodales. Curabitur elementum nibh nec bibendum pulvinar. Mauris mattis gravida scelerisque. Suspendisse
                accumsan lectus convallis, tempor purus id, varius nulla.</p>
        </div>
    </div>
</main>

<?php
require_once 'views/footer.php';
?>
</body>
</html>